...
class InfoDiv():
    def __init__(self, label, input_template, inner=True, data='0'):
        div_width = 200 if inner else 400
        label_width = 105 if inner else 310
        self._label_style = f'"font-weight: bold; margin-right: 5px; display: inline-block; width: {label_width}px;"'
        self._value_style = '"display: inline-block;"'
        ...
        self._value_div = Div(text=input_template)
...
class CurrentWeather():
    ...
    def _create_view(self):   
        ...
        info_grid = grid([self._air_temp.div, self._ground_temp.div, self._pressure.div, self._humidity.div, 
                          ...
                          ncols=2)
        self._cur_weather = column(self._header.div, info_grid, styles={'border': '1px solid gray', 'width': '400px', 
                                                                        'border-style': 'outset', 'background-color': 'lightblue'})

    def get_view(self):
        return self._cur_weather

    def update(self, dataframe):
        self._header.update(5)
